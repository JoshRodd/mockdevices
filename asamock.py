...
if sys.stdin.isatty() and sys.stdout.isatty():
    ...
    old_restore = old_settings
    tty.setraw(sys.stdin.fileno())

def atexit_termios():
    if old_restore is not None:
        termios.tcsetattr(sys.stdin.fileno(), termios.TCSADRAIN, old_restore)

import atexit
atexit.register(atexit_termios)
...
#    files = []
    ...
    for ln in s:
        for f in files:
            print((stamp + ' ' if f in (transscriptf, transscriptf2) else '') + ln, file=f, end='\r\n' if f == sys.stdout else '\n')
            f.flush()
import sys
...
'''
        ...
        if re.match(r'(^\s*(obj|acc|wr)|^\s*no)', ln):
