...
with open('asa.conf') as f:
    ...
    acl_re = re.compile('access-list (?P<acl>\S+) (?P<type>extended|remark (?P<remark>.*))( (?P<action>permit|deny) (?P<protocol>\d{1,3}|object-group (?P<svc_group>\S+)|[a-z]+) (?P<src>(<?P<src_network>([0-9]{1,3}\.){3}[0-9]{1,3} ([0-9]{1,3}\.){3}[0-9]{1,3})|host (?P<src_host>([0-9]{1,3}\.){3}[0-9]{1,3})|object-group (?P<src_group>\S+)|any?) (?P<dst>(?P<dst_network>([0-9]{1,3}\.){3}[0-9]{1,3} ([0-9]{1,3}\.){3}[0-9]{1,3})|host (?P<dst_host>([0-9]{1,3}\.){3}[0-9]{1,3})|object-group (?P<dst_group>\S+)|any?)( (?P<svc_type>(eq |gt |range |object-group )(?P<svc>.+)$))?)?')
    obj = 0
    object_group_name = ''
    for line in config:
...
for name,aces in ACLS.items():
    ...
    for ace in aces:
        ...
        if 'remark' in top_ace:
            ...
            output.append('{} (hitcnt=0) {}'.format(re.sub(r'access-list (\S+)',r'access-list \1 line {}'.format(line),top_ace.rstrip()),asa_hash(top_ace)))
